...
class ASPP(nn.Module):
    def __init__(self,
        ...
        self.conv2 = conv(
            depth * 5, depth, kernel_size=1, stride=1, bias=False)
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=1, dilation=2)

        self.aspp_motion = ASPP(
            512 * block.expansion,
            256,
            num_classes=2,
            conv=self.conv,
            norm=self.norm)
        self.aspp_modulation = ASPP(
            512 * block.expansion,
            256,
            num_classes=1,
            conv=self.conv,
            norm=self.norm)
        self.aspp_levelset = ASPP(
            512 * block.expansion,
            256,
            num_classes=1,
            conv=self.conv,
    ...
    def forward(self, x):
        ...
        motion_map = self.aspp_motion(feats)
        modulation_map = self.aspp_modulation(feats)
        levelset_map = self.aspp_levelset(feats)

        motion_map = nn.Upsample(size,
                                 mode='bilinear',
                                 align_corners=True)(motion_map)
        modulation_map = nn.Upsample(size,
                                 mode='bilinear',
                                 align_corners=True)(modulation_map)
        levelset_map = nn.Upsample(size,
                                 mode='bilinear',
                                 align_corners=True)(levelset_map)
        return motion_map, modulation_map, levelset_map
    ...
    if pretrained:
        ...
        if num_groups and weight_std:
            pretrained_dict = torch.load(
                'data/deeplab_resnet101_pascal_v3_bn_lr7e-3_epoch50.pth')
            overlap_dict = {
                k[7:]: v
