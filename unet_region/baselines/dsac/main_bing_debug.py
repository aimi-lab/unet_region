...
out_size = 64
if do_train:
    # data_path = '/home/krakapwa/data/single_buildings/train/'
    data_path = '/home/krakapwa/Desktop/data/single_buildings/'
else:
...
for i in range(num_ims):
    ...
    this_im = scipy.misc.imread(data_path + '/images/' + 'building_' + str(i) +
                                '.png')
    images[:, :, :, i] = np.float32(this_im) / 255
    img_mask = scipy.misc.imread(data_path + '/masks/' + 'building_mask_' +
                                 str(i) + '.png') / 65535
    masks[:, :, 0, i] = img_mask
...
GT = np.maximum(GT, 0)
plt.imshow(this_im);plt.scatter(poly[:,0], poly[:,1]);plt.show()
###########################################################################################
...
def epoch(n, i, mode):
    ...
    if mode is 'train':
        ...
        for j in range(len(batch_ind)):
            ...
            batch_mask[:, :, 0, j] = imrotate(
                batch_mask[:, :, 0, j], ang, resample='nearest')
    ...
    for j in range(batch_size):
        ...
        grads_arrayE[:, :, 0, 0] -= draw_poly(snake, 1, [M, N], 4) - draw_poly(
            thisGT, 1, [M, N], 4)
        grads_arrayA[:, :, 0, 0] -= (np.mean(der1) - np.mean(der1_GT))
        grads_arrayB[:, :, 0, 0] -= (draw_poly(snake, der2, [M, N], 4) -
...
with tf.Session(
    ...
    if do_train:
        ...
        polygons_csvfile = open(
            model_path + 'results/'
            'polygons.csv', 'a', newline='')
    ...
    for n in range(start_epoch, end_epoch):
        ...
        if do_train:
            for i in range(0, num_ims, batch_size):
                #print(i)
                #Do CNN inference
                new_iou, new_area_gt, new_area_snake, snake = epoch(
                    n, i, 'train')
                iou_train += new_iou
                iter_count += 1
                print('Train. Epoch ' + str(n) + '. Iter ' + str(iter_count) +
                      '/' + str(num_ims) +
                      ', IoU = %.2f' % (iou_train / iter_count))
            iou_train /= num_ims
        ...
        for i in range(num_ims):
            ...
            iou_test += new_iou
            iter_count += 1
            print('Test. Epoch ' + str(n) + '. Iter ' + str(iter_count) + '/' +
                  str(num_ims) + ', IoU = %.2f' % (iou_test / iter_count))
        ...
        if not do_train:
            iou_csvfile = open(
                model_path + 'iuo_train_test.csv', 'a', newline='')
