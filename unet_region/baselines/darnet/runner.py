...
def pretrain_vaihingen(split_num=1):
    ...
    checkpoint_path = pretrain.run(
        config['pretrain_vaihingen_{}'.format(split_num)], config,
        vaihingen.VaihingenDataset, DRNContours, split_num)
    overwrite_config('vaihingen_exp_{}'.format(split_num), 'restore',
                     checkpoint_path)
...
def pretrain_bing(split_num=1):
    ...
    checkpoint_path = pretrain.run(
        config['pretrain_bing_{}'.format(split_num)], config, bing.BingDataset,
        DRNContours, split_num)
    overwrite_config('bing_exp_{}'.format(split_num), 'restore',
                     checkpoint_path)
...
def train_baseline_vaihingen(split_num=1):
    ...
    checkpoint_path = train_baseline.run(
        config['baseline_vaihingen_{}'.format(split_num)], config,
        vaihingen.VaihingenDataset, DRNContours, split_num)
    overwrite_config('baseline_vaihingen_{}'.format(split_num), 'eval_model',
                     checkpoint_path)
...
def train_baseline_bing(split_num=1):
    ...
    checkpoint_path = train_baseline.run(
        config['baseline_bing_{}'.format(split_num)], config, bing.BingDataset,
        DRNContours, split_num)
    overwrite_config('baseline_bing_{}'.format(split_num), 'eval_model',
                     checkpoint_path)
...
def train_vaihingen(split_num=1):
    ...
    checkpoint_path = train_contours.run(
        config['vaihingen_exp_{}'.format(split_num)], config,
        vaihingen.VaihingenDataset, DRNContours, split_num)
    overwrite_config('vaihingen_exp_{}'.format(split_num), 'eval_model',
                     checkpoint_path)
...
def train_bing(split_num=1):
    ...
    checkpoint_path = train_contours.run(
        config['bing_exp_{}'.format(split_num)], config, bing.BingDataset,
        DRNContours, split_num)
    overwrite_config('bing_exp_{}'.format(split_num), 'eval_model',
                     checkpoint_path)
...
def eval_baseline_vaihingen(split_num=1):
    importlib.reload(dar_package.config)
    config = dar_package.config.config
    save_dir = eval_baseline.run(
        config['baseline_vaihingen_{}'.format(split_num)],
        vaihingen.VaihingenDataset, DRNContours)
...
def main(args):
    experiments = {'vaihingen': run_vaihingen, 'bing': run_bing}
    config = dar_package.config.config
