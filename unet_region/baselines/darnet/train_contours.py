...
class ModelAndLoss(torch.nn.Module):
    ...
    def forward(self, sample):
        ...
        loss, contour_x, contour_y, contour_rho = self.distance_loss(
            sample['init_contour'], sample['interp_radii'],
            sample['init_contour_origin'], beta, data, kappa,
            sample['interp_angles'], sample['delta_angles'])

        # Recover initial contour
        rho_cos_theta = sample['init_contour'] * torch.cos(
            sample['interp_angles'])
        rho_sin_theta = sample['init_contour'] * torch.sin(
            sample['interp_angles'])
...
def run(cfg_exp, config_object, Dataset, Network, split_num):
    ...
    for epoch_num in epoch_iterator:
        ...
        for sample in train_iterator:
            ...
            optimizer.zero_grad()
            loss, _, contour_x, contour_y, _, _, output = model_and_loss(
                sample)
            loss.backward()
            if 'grad_clip' in cfg_exp:
                torch.nn.utils.clip_grad_norm_(model_and_loss.parameters(),
                                               float(cfg_exp['grad_clip']))
            optimizer.step()
            global_iter += 1
